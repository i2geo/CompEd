...
public class ConcreteTopicDaoTest extends BaseDaoTestCase {
	
	private ConcreteTopicDao topicDao;
	private UserDao userDao;
	
	public void setConcreteTopicDao(ConcreteTopicDao topicDao) {
    ...
    public void testFindTopicByUri() throws Exception {
        Topic topic = topicDao.findByUri("%concreteTopic1%");
        assertNotNull(topic);
        assertEquals(new Long(172), topic.getId());
    }
    ...
    public void testFindByNameExact() throws Exception {
    	...
    	List<ConcreteTopic> list = topicDao.findByNameExact(token, language);
    ...
    public void testFindByNameLike() throws Exception {
    	...
    	List<ConcreteTopic> list = topicDao.findByNameLike(token, language);
	...
	*/
    ...
    public void testFindByLastCreated() throws Exception {
    	...
    	List<ConcreteTopic> topics = topicDao.findByLastCreated(number);
    	assertNotNull(topics);
    	assertTrue(topics.size() > 0);
    	assertTrue(topics.size() <= number);
    
    	assertTrue(172L == topics.get(0).getId());
    }
    ...
    public void testFindByLastModified() throws Exception {
    	...
    	List<ConcreteTopic> topics = topicDao.findByLastModified(number);
    	assertNotNull(topics);
    	assertTrue(topics.size() > 0);
    	assertTrue(topics.size() <= number);
